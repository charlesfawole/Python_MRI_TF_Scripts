function [] = Val_TF_plot_new_303()

% mT_tip = [6.75	5.05	2.68	1.15	0.91	0.79	8.85	5.85	2.75];
% mT_ring = [4.12	2.68	1.47	5.68	3.79	2.35	8.28	5.63	3.03];

mT_tip = [7.48	5.20	2.92	1.42	1.15	1.09	9.55	6.34	2.96];
mT_ring = [4.87	2.92	1.71	5.65	3.85	2.68	8.94	6.05	3.27];


mV_tip = [2.60	2.50	2.40	2.65	1.73	1.65	3.10	1.61	1.33];
mV_ring = [3.10	2.50	2.20	2.97	1.30	1.30	3.90	2.25	1.09];
...
%% temperature tip
cT_tip = Val_TF_new_303(p_coil, ratio, p_gel, 1, 'T', 'tip', dx_lead,dy_lead,dz_lead, 0.2, 'extrap');
%[~,idx] = max(mT_tip);
...
%% temperature ring
cT_ring = Val_TF_new_303(p_coil, ratio, p_gel, 1, 'T', 'ring', dx_lead,dy_lead,dz_lead, 0.2, 'extrap');
% [~,idx] = max(mT_ring);
...
% %% voltage tip
cV_tip = Val_TF_new_303(p_coil, ratio, p_gel, 1, 'V', 'tip', dx_lead,dy_lead,dz_lead, 0.2, 'extrap');
% [~,idx] = max(mV_tip);
...
% 
% %% voltage ring
cV_ring = Val_TF_new_303(p_coil, ratio, p_gel, 1, 'V', 'ring', dx_lead,dy_lead,dz_lead, 0.2, 'extrap');
% [~,idx] = max(mV_ring);
